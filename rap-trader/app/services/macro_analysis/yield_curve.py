"""Deterministic yield-curve analysis service.

Classifies the 2Y/10Y spread (or any yield-curve series) into a
``YieldCurveTrend`` (inverted, normal, flat, or steepening).
"""
...
from app.services.macro_analysis.domain import YIELD_CURVE_CATEGORY
from app.services.macro_analysis.observations import MacroObservation
from app.services.macro_analysis.trends import _map_yield_curve


class YieldCurveAnalysisService(MacroAnalysisService):
    category = YIELD_CURVE_CATEGORY
    ...
    def classify(self, observations: Sequence[MacroObservation], evaluated_at: datetime) -> MacroSignal | None:
        ...
        trend = _map_yield_curve(delta, latest.value, self.config)
        confidence = self.config.base_evidence_confidence
        return MacroSignal(
            signal_id=build_signal_id(self.category, latest.series_id, evaluated_at),
            category=self.category,
            label=f"yield curve {trend.value.lower()} (spread={latest.value:+.2f}{latest.units})",
