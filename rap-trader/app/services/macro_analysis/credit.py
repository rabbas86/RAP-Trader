"""Deterministic credit-conditions analysis service.

Classifies the credit-spread series into a ``CreditCondition``
(tightening, loosening, or stable).
"""
...
from app.services.macro_analysis.domain import CREDIT_CATEGORY
from app.services.macro_analysis.observations import MacroObservation
from app.services.macro_analysis.trends import _map_credit


class CreditAnalysisService(MacroAnalysisService):
    ...
    def classify(self, observations: Sequence[MacroObservation], evaluated_at: datetime) -> MacroSignal | None:
        ...
        trend = _map_credit(latest.value, self.config)
        confidence = self.config.base_evidence_confidence
        return MacroSignal(
            signal_id=build_signal_id(self.category, latest.series_id, evaluated_at),
            category=self.category,
            label=f"credit conditions {trend.value.lower()} (spread={latest.value:.2f}{latest.units})",
