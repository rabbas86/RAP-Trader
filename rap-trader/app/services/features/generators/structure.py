"""Causal market-structure detection extracted from Phase 6."""
...
class StructureFeatureGenerator:
    version = "1.0.0"
    minimum_bars = 5
    ...
    def generate(bars: list[OHLCVBar]) -> dict[str, FeatureScalar]:
        ...
        state = classify_structure(bars, swings)
        return {
            "structure.bos_timestamp": state.bos_timestamp.isoformat() if state.bos_timestamp else None,
            "structure.choch_timestamp": state.choch_timestamp.isoformat() if state.choch_timestamp else None,
            "structure.higher_highs": state.higher_highs,
            "structure.higher_lows": state.higher_lows,
            "structure.lower_highs": state.lower_highs,
            "structure.lower_lows": state.lower_lows,
            "structure.regime": state.regime,
            "structure.swing_count": len(swings),
        }
